...
- Installed usage should go through the `acpella` executable. The source checkout is a development location where `pnpm cli` still works.
- Acpella loads `${XDG_CONFIG_HOME:-~/.config}/acpella/.env` by default; `--env-file <path>` overrides that for one invocation.
...
Command examples use the installed `acpella` executable. From the source checkout during development, `pnpm cli` is the equivalent fallback.
