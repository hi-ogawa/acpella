...
console.log(
  JSON.stringify(
    { imported: true, mode, baseUrl: baseUrl ?? null, api: summarizeClient(client) },
    null,
    2,
  ),
);
...
async function smoke(next: OpencodeClient) {
  const health = await next.global.health({ throwOnError: true });
  console.log(JSON.stringify({ health: health.data }, null, 2));

  const sessions = await next.session.list({ limit: 1 }, { throwOnError: true });
  console.log(
    JSON.stringify({ sessionListOk: true, sessionCount: sessions.data?.length ?? 0 }, null, 2),
  );
}
...
if (mode === "events") {
  await withServer(async (next) => {
    ...
    console.log(
      JSON.stringify(
        { sessionCreated: { id: session.data.id, title: session.data.title } },
        null,
        2,
      ),
    );

    const messages = await next.session.messages(
      { sessionID: session.data.id },
      { throwOnError: true },
    );
    console.log(
      JSON.stringify({ messagesOk: true, messageCount: messages.data?.length ?? 0 }, null, 2),
    );
