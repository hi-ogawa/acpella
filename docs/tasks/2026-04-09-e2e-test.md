...
- `ACPELLA_TELEGRAM_API_ROOT` — point grammy at a local fake HTTP server instead of `api.telegram.org`
- `ACPELLA_AGENT=fake-echo` — point acpx at a trivial ACP-compatible agent

## Fake 1: Telegram API server

grammy supports `new Bot(token, { client: { apiRoot } })`.

**Production code change**: read `ACPELLA_TELEGRAM_API_ROOT` env var, pass to Bot constructor. ~3 lines.
...
2. Start daemon as child process:
     ACPELLA_TELEGRAM_BOT_TOKEN=test-token
     ACPELLA_TELEGRAM_API_ROOT=http://localhost:<port>
     ACPELLA_AGENT=fake-echo
     ACPELLA_TELEGRAM_ALLOWED_USER_IDS=456
     ACPELLA_HOME=/tmp/test
...
1. `src/index.ts`: read `ACPELLA_TELEGRAM_API_ROOT` env var, pass as `apiRoot` to `Bot` constructor
