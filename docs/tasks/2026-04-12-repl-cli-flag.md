...
- `src/index.ts` loads config, chooses real Telegram bot vs in-process test bot, and starts either polling or the REPL.
...
1. Parse a small CLI options object in `src/index.ts` from `process.argv.slice(2)`.
